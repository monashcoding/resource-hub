...
// mac-auth token verification. Mirrors the canonical verifier the auth
...
// Admin surfaces (region/category/resource CRUD) are exec/committee. `member` is a normal member, not admin.
const ADMIN_ROLES = (process.env.MAC_ADMIN_ROLES ?? 'exec,committee')
  .split(',')
  .map((r) => r.trim().toLowerCase())
  .filter(Boolean);

/** Require an exec/committee role claim. */
