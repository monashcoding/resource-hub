...
# Drop root. The app listens on 3000 (no privileged port) and writes nothing to
# disk, so there is nothing for root to do here except widen the blast radius on
# a shared VM. `node` is a uid that already exists in the base image.
USER node
